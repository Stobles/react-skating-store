...
const BlogCard = ({ src, isList, category, blogId, title }) => (
  <li>
      ...
      <div className={isList ? `${styles.Content} ${styles.List}` : styles.Content}>
        ...
        {isList && (
          <div className={styles.Description}>
            ваоироыдаааааапаоывАЛОлоровл
            РАолАОДЛЫРАоралыолРАлоролыаОВРАоы
            ВАролрарадЛРОЛарлраОЛРАжлРАлрВЛарворво
            ЛРАоларв....
          </div>
        )}
...
const BlogList = ({ blogs, isLoading, isList }) => (
  <ul className={isList ? `${styles.Cards} ${styles.List}` : styles.Cards}>
    ...
    {blogs.map(({ id, img, title, category, date }) => (
      <BlogCard
        key={id}
        blogId={id}
        isList={isList}
